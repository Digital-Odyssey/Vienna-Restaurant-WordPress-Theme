<?php 
	...
	$searchFieldText = get_theme_mod('searchFieldText', esc_html__( 'Type Keywords...', 'viennatheme' )); 
	...
	<?php if($enableSearch == 'on') { ?>
		<div class="pm-mobile-global-menu-search">
            <form action="<?php echo home_url( '/' ); ?>" method="get" id="pm-searchform" name="searchform">
                <input name="s" type="text" class="pm-search-field-mobile" placeholder="<?php echo esc_html__(esc_attr($searchFieldText),'viennatheme'); ?>">
            </form>
        </div>
    <?php } ?>
