...
<?php 
	...
	$globalContainerPadding = get_theme_mod('globalContainerPadding', 0);
	...
	if( $globalContainerPadding > 0 ) {
		$bootstrapContainerPadding = $globalContainerPadding;
	}
...
<?php if($pageLayout === 'no-sidebar') { //Render col-lg-12 ?>
		...
		<?php if($disableContainer === 'yes') { ?>
        	...
        	<?php if (have_posts()) :  while (have_posts()) : the_post(); ?>
		    	<?php the_content(); ?>
            <?php endwhile; else: ?>
            	<p><?php echo esc_html_e('No content was found.', 'viennatheme'); ?></p>
            <?php endif; ?>
