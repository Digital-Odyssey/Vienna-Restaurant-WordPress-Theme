		  ...
		  echo '<div class="pm-micro-nav-lang-selector desktop">';
			 ...
			 echo '<div class="pm-dropdown pm-language-selector-menu">';
				 echo '<div class="pm-dropmenu">';
					 echo '<p class="pm-menu-title">'.esc_html__('Language','viennatheme').'</p>';
...
function pm_ln_breadcrumbs() {
    ...
    if (!is_home()) {
		...
		if (is_single() && get_post_type() == 'post_staff' ) { //Wordpress doesnt support custom post types for breadcrumbs
            ...
            echo '<p> '.esc_html__('404 Error', 'viennatheme').'</p>';
...
function pm_ln_mytheme_comment($comment, $args, $depth) {
    ...
    <div id="comment-<?php comment_ID(); ?>">
        ...
        <div class="pm-comment-box-avatar-container">
            ...
            <ul class="pm-comment-author-list">
                <li><p class="pm-comment-name"><?php printf(esc_html__('%s', 'viennatheme'), get_comment_author_link()) ?> <a href="<?php echo htmlspecialchars(get_comment_link( $comment->comment_ID )) ?>"></a></p></li>
                <li><p class="pm-comment-date"><?php printf(esc_html__('%1$s at %2$s', 'viennatheme'), get_comment_date(),get_comment_time()) ?><?php edit_comment_link(esc_html__('(Edit)', 'viennatheme'),' ','') ?></p></li>
            </ul>
        ...
        <div class="pm-comment">
        	...
        	<?php if ($comment->comment_approved == '0') : ?>
				<p class="pm-primary" style="font-style:italic;"><?php esc_html_e('Your comment is awaiting moderation.', 'viennatheme') ?></p>
            <?php endif; ?>
	...
	echo '<div class="pm-comment-reply-form">';
		...
		$comments_args = array(
		  'title_reply'       => esc_html__( 'Leave a Reply', 'viennatheme' ),
		  'title_reply_to'    => esc_html__( 'Leave a Reply to %s', 'viennatheme' ),
		  'cancel_reply_link' => esc_html__( 'Cancel Reply', 'viennatheme' ),
		  'label_submit'      => esc_html__( 'Post Comment', 'viennatheme' ),
		);
...
function pm_ln_send_contact_form() {
	...
	$headers .= 'From: '.esc_attr__('donotreply', 'viennatheme').'@'. $_SERVER['SERVER_NAME'] .' <donotreply@'. $_SERVER['SERVER_NAME'] .'>' . "\r\n";
	
	$subj = esc_html__('Contact Form Inquiry', 'viennatheme');
	
	$body = ' 
	
	  **** '. esc_html__('THIS IS AN AUTOMATED MESSAGE. PLEASE DO NOT REPLY DIRECTLY TO THIS EMAIL', 'viennatheme') .' ****
	  
	  '. esc_html__('Name', 'viennatheme') .': '.$full_name.'
	  '. esc_html__('Email Address', 'viennatheme') .': '.$email_address.'
	  '. esc_html__('Subject', 'viennatheme') .': '.$subject.'
	  '. esc_html__('Message', 'viennatheme') .': '.$message.'
...
function pm_ln_send_quick_contact_form() {
	...
	$subj = esc_html__('Quick Contact Form Inquiry', 'viennatheme');
	
	$body = ' 
	
	  **** '. esc_html__('THIS IS AN AUTOMATED MESSAGE. PLEASE DO NOT REPLY DIRECTLY TO THIS EMAIL', 'viennatheme') .' ****
	  
	  '. esc_html__('Full Name', 'viennatheme') .': '.$full_name.'
	  '. esc_html__('Email Address', 'viennatheme') .': '.$email_address.'
	  '. esc_html__('Message', 'viennatheme') .': '.$message.'
...
function pm_ln_send_catering_form() {
	...
	$subj = esc_html__('Catering Form Inquiry', 'viennatheme');
	
	$body = ' 
	
	  **** '. esc_html__('THIS IS AN AUTOMATED MESSAGE. PLEASE DO NOT REPLY DIRECTLY TO THIS EMAIL', 'viennatheme') .' ****
	  
	  '. esc_html__('First Name', 'viennatheme') .': '.$first_name.'
	  '. esc_html__('Last Name', 'viennatheme') .': '.$last_name.'
	  '. esc_html__('Email Address', 'viennatheme') .': '.$email_address.'
	  '. esc_html__('Phone Number', 'viennatheme') .': '.$phone_number.'
	  '. esc_html__('Event Type', 'viennatheme') .': '.$event_type.'
	  '. esc_html__('Event Date', 'viennatheme') .': '.$date.'
	  '. esc_html__('Number of Guests', 'viennatheme') .': '.$num_of_guests.'
	  '. esc_html__('Location', 'viennatheme') .': '.$location.'
	  '. esc_html__('Time', 'viennatheme') .': '.$time.'
	  '. esc_html__('Additional Information', 'viennatheme') .': '.$info.'
...
function pm_ln_send_event_form() {
	...
	$subj = esc_html__('Event Form Inquiry', 'viennatheme');
...
function pm_ln_send_reservation_form() {
	...
	$subj = esc_html__('Reservation Form Inquiry', 'viennatheme');
	...
	$body = ' 
	  ...
	  **** '. esc_html__('THIS IS AN AUTOMATED MESSAGE. PLEASE DO NOT REPLY DIRECTLY TO THIS EMAIL', 'viennatheme') .' ****
	  ...
	  '. esc_html__('Email Address', 'viennatheme') .': '.$email_address.'
	  '. esc_html__('Phone Number', 'viennatheme') .': '.$phone_number.'
	  '. esc_html__('Date of Reservation', 'viennatheme') .': '.$date.'
	  '. esc_html__('Number of seats', 'viennatheme') .': '.$num_of_seats.'
	  '. esc_html__('Restaurant Location', 'viennatheme') .': '.$location.'
	  '. esc_html__('Reservation Time', 'viennatheme') .': '.$time.'
	  '. esc_html__('Additional Information', 'viennatheme') .': '.$info.'
