...
class pm_eventposts_widget extends WP_Widget {
	...
	function pm_eventposts_widget() {
		...
		$widget_ops = array(
			...
			'description' => esc_html__('Display your events.','viennatheme')
		);
		
		parent::__construct('pm_eventposts_widget', esc_html__('[Micro Themes] - Events','viennatheme'), $widget_ops);
	...
	function form($instance){
		...
		?>
            ...
            <p>
                <!--<label for="<?php //echo $this->get_field_id( 'fa_icon' ); ?>"><?php //esc_html_e('FontAwesome Icon:', 'viennatheme') ?></label>
            ...
            <p><?php esc_html_e('Number of Events to show:', 'viennatheme') ?> <input class="widefat" name="<?php echo esc_attr($this->get_field_name('numOfPosts')); ?>" type="text" value="<?php echo esc_attr($numOfPosts); ?>" /></p>
            
            <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'orderBy' )); ?>"><?php esc_html_e('Order By:', 'viennatheme') ?></label>
            <select id="<?php echo esc_attr($this->get_field_id( 'orderBy' )); ?>" name="<?php echo $this->get_field_name( 'orderBy' ); ?>" class="widefat">
                <option <?php if ( 'post_date' == $instance['orderBy'] ) echo 'selected="selected"'; ?> value="post_date"><?php esc_html_e('Date Published', 'viennatheme') ?></option>
                <option <?php if ( 'event_date' == $instance['orderBy'] ) echo 'selected="selected"'; ?> value="event_date"><?php esc_html_e('Event Date', 'viennatheme') ?></option>
            </select>
            </p>
            
            <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'displayExpired' )); ?>"><?php esc_html_e('Display expired events?', 'viennatheme') ?></label>
            <select id="<?php echo esc_attr($this->get_field_id( 'displayExpired' )); ?>" name="<?php echo $this->get_field_name( 'displayExpired' ); ?>" class="widefat">
                <option <?php if ( 'no' == $instance['displayExpired'] ) echo 'selected="selected"'; ?> value="no"><?php esc_html_e('NO', 'viennatheme') ?></option>
                <option <?php if ( 'yes' == $instance['displayExpired'] ) echo 'selected="selected"'; ?> value="yes"><?php esc_html_e('YES', 'viennatheme') ?></option>
            </select>
            </p>
	...
	function widget($args, $instance){
		...
		*/
		...
		if ($published_posts >= 1) {
					...
					echo '<div class="pm-event-widget-container">';
						...
						echo '<div class="pm-event-widget-desc">';
							...
							if($pm_event_start_time_meta !== '' && $pm_event_end_time_meta !== '') :
								...
								echo '<p class="pm-event-item-times"><strong class="pm-primary">'. esc_html__('START:', 'viennatheme') .'</strong> '. esc_attr($pm_event_start_time_meta) .' <strong class="pm-primary">'. esc_html__('END:', 'viennatheme') .'</strong> '. esc_attr($pm_event_end_time_meta) .'</p>';
						...
						echo '<ul class="pm-event-widget-btns">';
							echo '<li><a href="'.$recent['guid'].'" class="pm-rounded-btn small">'.esc_html__('More Info','viennatheme').'</a></li>';
							echo '<li><a href="'.esc_html($pm_event_fan_page_meta).'" class="pm-rounded-btn small event-fan-page"><i class="fa fa-facebook"></i> &nbsp;'.esc_html__('Fan Page','viennatheme').'</a></li>';
						echo '</ul>';
					echo '</div>';
			...
			echo '<p>'. esc_html__('All events have expired.', 'viennatheme') .'</p>';
